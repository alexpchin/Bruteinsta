...
dependencies() {
    ...
    command -v openssl >/dev/null 2>&1 || {
        echo >&2 "I require openssl but it's not installed. Aborting."
        exit 1
    }
    command -v tor >/dev/null 2>&1 || {
        echo >&2 "I require tor but it's not installed. Aborting."
        exit 1
    }
    ...
    command -v awk >/dev/null 2>&1 || {
        echo >&2 "I require awk but it's not installed. Aborting."
        exit 1
    }
    command -v sed >/dev/null 2>&1 || {
        echo >&2 "I require sed but it's not installed. Aborting."
        exit 1
    }
    command -v cat >/dev/null 2>&1 || {
        echo >&2 "I require cat but it's not installed. Aborting."
        exit 1
    }
    ...
    command -v wc >/dev/null 2>&1 || {
        echo >&2 "I require wc but it's not installed. Aborting."
        exit 1
    }
    command -v cut >/dev/null 2>&1 || {
        echo >&2 "I require cut but it's not installed. Aborting."
        exit 1
    }
    command -v uniq >/dev/null 2>&1 || {
        echo >&2 "I require uniq but it's not installed. Aborting."
        exit 1
    }
    if [ $(
        ls /dev/urandom >/dev/null
        echo $?
    ) == "1" ]; then
...
function start() {
    ...
    if [[ "$checkaccount" == 1 ]]; then
        ...
        default_wl_pass="passwords2.lst"
...
checktor() {
    ...
    check=$(
        curl --socks5 localhost:9050 -s https://check.torproject.org >/dev/null
        echo $?
    )
...
function bruteforcer() {
    ...
    while [ true ]; do
        ...
        for pass in $(sed -n ''$startline','$endline'p' "$wl_pass"); do
            ...
            # Increment the counter here to get the correct password attempt count
            countpass=$((startline++))
...
#                     printf "\e[1;92m [*] Saved:\e[0m\e[1;77m found.passwords \n\e[0m"
#                     kill -1 $$
#                 elif [[ $var == "Please wait" ]]; then changeip; fi
#             ); } &
#         done
#         wait $!
